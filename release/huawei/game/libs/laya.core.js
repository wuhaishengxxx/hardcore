		...
		__proto.runEvent = function () {
			...
			while (i < len) {
				...
				try {
					...
					this._eventList.length = 0;	// 后面不在执行，不然会越积越多
					throw e;
				}
	...
	var ResourceVersion = (function () {
		...
		ResourceVersion.addVersionPrefix = function (originURL) {
			...
			if (ResourceVersion.manifest) {
				if (ResourceVersion.manifest[originURL]) {
					return ResourceVersion.manifest[originURL] + "/" + originURL;
				}

				if (originURL.indexOf('res/') == 0) {
					return "1/" + originURL;
				}
			}
			return originURL;
			...
			if (hbs && !url.startsWith("http")) {
				...
				setTimeout(() => {
					...
					if (type == 'pkm' || type === "arraybuffer") {
						response = hbs.getFileSystemManager().readFileSync(url, 'binary');
					} else {
						response = hbs.getFileSystemManager().readFileSync(url, 'utf8');
	...
	*/
	...
	var AudioSoundChannel = (function (_super) {
		...
		__proto.__resumePlay = function () {
			...
			try {
				this._audio && (this._audio.currentTime = this.startTime);
		...
		*/
		__proto.play = function () {
			...
			try {
				...
				this._audio && this._audio.addEventListener("canplay", this._resumePlay);
				return;
			}
			SoundManager.addChannel(this);
			this._audio && Browser.container.appendChild(this._audio);
		...
		*/
		__proto.stop = function () {
			...
			this._audio && this._audio.removeEventListener("ended", this._onEnd);
			this._audio && this._audio.removeEventListener("canplay", this._resumePlay);
			if (!Browser.onIE) {
				if (this._audio && this._audio != AudioSound._musicAudio) {
