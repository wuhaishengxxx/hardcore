		...
		*/
		__proto.layout = function () {
			...
			if (tArray) {
				...
				// 丁伟加的 各版本通用，会影响外网 就不去掉了
				if (this._getCSSStyle()._getValign() == 1) {
					for (var i = 0; i < this._childs.length; i++) {
						var e = this._childs[i];
						if (e instanceof HTMLElement && e._text.words) {
							for (var j = 0; j < e._text.words.length; j++) {
								var w = e._text.words[j];
								w.y = (this.height - tArray[1]) / 2 + w.y;
							}
						} else {
							e.y = (this.height - tArray[1]) / 2 + e.y;
						}
					}
				}
			}
